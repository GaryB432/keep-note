...
cli
  ...
  .action(async (dir, options) => {
    const allNotes = await digest(dir);

    const labelsUsed = allNotes.reduce((a, note) => {
      if (note.labels) {
        note.labels.forEach((l) => a.add(l.name));
      }

      return a;
    }, new Set());

    const fun = await select({
      message: "skip any labels",
      options: Array.from(labelsUsed.values()).map(d=> ({value: d})),
    });
    ...
    // console.log(JSON.stringify(nns, undefined, 4));
